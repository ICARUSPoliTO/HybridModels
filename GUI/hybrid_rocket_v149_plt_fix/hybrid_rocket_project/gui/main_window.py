...
class HybridRocketGUI:
    ...
    def show_mission_output_page(self):
        ...
        # Get Gox limits from optimization page if available
        gox_min = 100.0
        gox_max = 800.0
        try:
            if 'Optimization_Gox_min' in self.inputs:
                gox_min = float(self.inputs['Optimization_Gox_min'].get())
            if 'Optimization_Gox_max' in self.inputs:
                gox_max = float(self.inputs['Optimization_Gox_max'].get())
        except:
            pass
        ...
        if hasattr(self.controller, 'mission_results') and self.controller.mission_results:
            ...
            self.current_page_obj.display_results(
                ...
                gox_min=gox_min,
                gox_max=gox_max,
                geometry_history=mr.get('geometry_history', None)
            )
    ...
    def create_mission_buttons(self):
        ...
        # Save Mission Parameters button
        tk.Button(
            button_frame,
            text="Save Mission Parameters",
            font=FONTS['button'],
            bg=COLORS['button_inactive'],
            fg='black',
            command=self.save_mission_parameters,
            cursor='hand2',
            padx=15, pady=8
        ).grid(row=0, column=0, padx=10)
        
        # Load Mission Parameters button
        tk.Button(
            button_frame,
            text="Load Mission Parameters",
            font=FONTS['button'],
            bg=COLORS['button_inactive'],
            fg='black',
            command=self.load_mission_parameters,
            cursor='hand2',
            padx=15, pady=8
        ).grid(row=0, column=1, padx=10)
    ...
    def save_mission_parameters(self):
        """Save mission parameters to CSV file"""
        try:
            ...
            # Collect all mission inputs
            mission_data = {}
            for key, widget in self.inputs.items():
                if key.startswith('Mission_'):
                    try:
                        mission_data[key] = widget.get()
                    except:
                        pass
            
            # Add dropdown values
            if hasattr(self, 'mission_page_obj') and self.mission_page_obj:
                if self.mission_page_obj.grain_preset_var:
                    mission_data['Mission_grain_preset'] = self.mission_page_obj.grain_preset_var.get()
                if self.mission_page_obj.tank_type_var:
                    mission_data['Mission_tank_type'] = self.mission_page_obj.tank_type_var.get()
                if self.mission_page_obj.circular_var:
                    mission_data['Mission_circular'] = str(self.mission_page_obj.circular_var.get())
            
            import csv
            with open(filepath, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Parameter', 'Value'])
                for key, value in sorted(mission_data.items()):
                    writer.writerow([key, value])
            
            messagebox.showinfo("Success", f"Mission parameters saved to:\n{filepath}")
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save parameters:\n{str(e)}")
    
    def load_mission_parameters(self):
        """Load mission parameters from CSV file"""
        try:
            ...
            if hasattr(self, 'mission_page_obj') and self.mission_page_obj:
                if 'Mission_grain_preset' in mission_data and self.mission_page_obj.grain_preset_var:
                    self.mission_page_obj.grain_preset_var.set(mission_data['Mission_grain_preset'])
                    self.mission_page_obj._on_grain_preset_change()
                if 'Mission_tank_type' in mission_data and self.mission_page_obj.tank_type_var:
                    self.mission_page_obj.tank_type_var.set(mission_data['Mission_tank_type'])
                    self.mission_page_obj._on_tank_type_change()
                if 'Mission_circular' in mission_data and self.mission_page_obj.circular_var:
                    self.mission_page_obj.circular_var.set(mission_data['Mission_circular'] == 'True')
            
            messagebox.showinfo("Success", f"Mission parameters loaded from:\n{filepath}")
    ...
    def _show_mission_progress_dialog(self, title: str):
        ...
        self.mission_progress_window.geometry("450x250")
        ...
        self.mission_progress_window.geometry(f"450x250+{x}+{y}")
        ...
        self.mission_stop_button.pack(pady=20)
    ...
    def show_running_dialog(self, inputs=None):
        ...
        self.running_dialog.geometry("500x320")
        ...
        y = (self.running_dialog.winfo_screenheight() // 2) - (160)
        self.running_dialog.geometry(f'500x320+{x}+{y}')
