...
class OptimizationOutputPage:
    ...
    TAB_DEFINITIONS = {
        ...
        'Combustion': {
            'variables': ['MR_array', 'Tc_array', 'Gox_array', 'r_array'],
            'labels': {
                'MR_array': 'MR (O/F)',
                'Tc_array': 'Tc [K]',
                'Gox_array': 'Gox [kg/(s·m²)]',
                'r_array': 'r [m/s]'
            },
            'units': {
                'MR_array': '-',
    ...
    def update_tab_plots(self, tab_name):
        ...
        for i, var_name in enumerate(variables):
            ...
            # Labels and title
            ax.set_xlabel('Dport/Dt', fontsize=9)
            ax.set_ylabel('Dinj/Dt', fontsize=9)
            ax.set_title(labels.get(var_name, var_name), fontsize=10, fontweight='bold')
    ...
    def display_results(self, results, ranges=None):
        """
    ...
    def _update_text_results(self):
        ...
        if 'Ivac_array' in self.results:
            ...
            if len(valid_indices) > 0:
                ...
                # Add Gox for optimal configuration
                gox_arr = self.results.get('Gox_array', np.zeros_like(ivac))
                if gox_arr.size > 0 and gox_arr[i,j,k] > 0:
                ...
                # ================================================================
                # GOX RANGE (for blow-off / extinction limits check)
                # ================================================================
                self.results_text.insert(tk.END, "=" * 40 + "\n")
                self.results_text.insert(tk.END, "GOX OPERATING RANGE\n")
                self.results_text.insert(tk.END, "=" * 40 + "\n\n")
                
                if gox_arr.size > 0:
                    valid_gox = gox_arr[gox_arr > 0]
                    if len(valid_gox) > 0:
                        gox_min = np.min(valid_gox)
                        gox_max = np.max(valid_gox)
                        gox_mean = np.mean(valid_gox)
                        self.results_text.insert(tk.END, f"Gox_min  = {gox_min:.2f} kg/(s·m²)\n")
                        self.results_text.insert(tk.END, f"Gox_max  = {gox_max:.2f} kg/(s·m²)\n")
                        self.results_text.insert(tk.END, f"Gox_mean = {gox_mean:.2f} kg/(s·m²)\n")
                        self.results_text.insert(tk.END, "\n")
                        self.results_text.insert(tk.END, "Check these values against:\n")
                        self.results_text.insert(tk.END, "• Blow-off limit (Gox too high)\n")
                        self.results_text.insert(tk.END, "• Extinction limit (Gox too low)\n")
                        self.results_text.insert(tk.END, "\n")
                    else:
            ...
            self.results_text.insert(tk.END, "Ivac_array not available.\n\n")
