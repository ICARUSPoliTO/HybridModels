...
class MissionOutputPage:
    ...
    def create_summary_section(self, parent):
        ...
        def safe_first(lst):
            if lst and len(lst) > 0 and lst[0] is not None:
                return lst[0]
            return 0
        
        def safe_last(lst):
            if lst and len(lst) > 0 and lst[-1] is not None:
                return lst[-1]
            return 0
        ...
        # Calculate active burn time (when Thrust > 1% of max)
        thrust_list = self.results.get('Thrust', [])
        burn_list = self.results.get('burn', [])
        max_thrust = safe_max(thrust_list)
        thrust_threshold = max_thrust * 0.01 if max_thrust > 0 else 0
        
        # Find burn start and end times
        burn_start_time = 0
        burn_end_time = total_time
        for i, t in enumerate(self.time_data):
            if i < len(thrust_list) and thrust_list[i] is not None and thrust_list[i] > thrust_threshold:
                burn_start_time = t
                break
        for i in range(len(self.time_data) - 1, -1, -1):
            if i < len(thrust_list) and thrust_list[i] is not None and thrust_list[i] > thrust_threshold:
                burn_end_time = self.time_data[i]
                break
        
        active_burn_time = burn_end_time - burn_start_time
        ...
        # === MOTOR GEOMETRY ===
        D_chamber = safe_first(self.results.get('D_chamber', [0.1]))
        Dt = np.sqrt(4 * safe_first(self.results.get('At', [7.07e-4])) / np.pi)
        Lc = safe_first(self.results.get('Lc', [0.1]))
        Ainj = safe_first(self.results.get('Ainj', [1e-4]))
        eps = safe_first(self.results.get('eps', [4.0]))
        
        # Port area: initial and final
        Ap_list = self.results.get('Ap', [])
        Ap_initial = safe_first(Ap_list)
        Ap_final = safe_last(Ap_list)
        Dp_initial = np.sqrt(4 * Ap_initial / np.pi) if Ap_initial > 0 else 0
        Dp_final = np.sqrt(4 * Ap_final / np.pi) if Ap_final > 0 else 0
        
        # Calculate D_inj from Ainj (assuming single injector or total area)
        D_inj = np.sqrt(4 * Ainj / np.pi) if Ainj > 0 else 0
        
        # Build summary with clear sections
        row = 0
        
        # --- TIMING SECTION ---
        self._add_section_header(section, "⏱️ TIMING", row)
        row += 1
        row = self._add_summary_row(section, "Total Time", f"{total_time:.3f} s", row)
        row = self._add_summary_row(section, "Active Burn Time", f"{active_burn_time:.3f} s", row)
        row = self._add_summary_row(section, "Burn Start", f"{burn_start_time:.3f} s", row)
        row = self._add_summary_row(section, "Burn End", f"{burn_end_time:.3f} s", row)
        row += 1
        
        # --- PERFORMANCE SECTION ---
        self._add_section_header(section, "🚀 PERFORMANCE", row)
        row += 1
        row = self._add_summary_row(section, "Max Thrust", f"{safe_max(thrust_list):.1f} N", row)
        row = self._add_summary_row(section, "Mean Thrust", f"{safe_mean(thrust_list):.1f} N", row)
        row = self._add_summary_row(section, "Max pc", f"{safe_max(pc_list)/1e5:.2f} bar", row)
        row = self._add_summary_row(section, "Mean pc", f"{safe_mean(pc_list)/1e5:.2f} bar", row)
        row = self._add_summary_row(section, "Max Isp", f"{safe_max(is_list):.1f} s", row)
        row = self._add_summary_row(section, "Mean Isp", f"{safe_mean(is_list):.1f} s", row)
        row += 1
        
        # --- PROPELLANT SECTION ---
        self._add_section_header(section, "⛽ PROPELLANT", row)
        row += 1
        row = self._add_summary_row(section, "Oxidizer Used", f"{safe_max(mL_list) - safe_min(mL_list):.3f} kg", row)
        row = self._add_summary_row(section, "Fuel Used", f"{safe_max(m_fuel_list) - safe_min(m_fuel_list):.3f} kg", row)
        row += 1
        
        # --- MOTOR GEOMETRY SECTION ---
        self._add_section_header(section, "📐 MOTOR GEOMETRY", row)
        row += 1
        row = self._add_summary_row(section, "D_chamber", f"{D_chamber*1000:.2f} mm", row)
        row = self._add_summary_row(section, "Dt (throat)", f"{Dt*1000:.2f} mm", row)
        row = self._add_summary_row(section, "Lc (grain length)", f"{Lc*1000:.2f} mm", row)
        row = self._add_summary_row(section, "ε (expansion)", f"{eps:.2f}", row)
        row = self._add_summary_row(section, "D_inj (total equiv.)", f"{D_inj*1000:.2f} mm", row)
        row += 1
        
        # --- PORT GEOMETRY SECTION ---
        self._add_section_header(section, "🔄 PORT EVOLUTION", row)
        row += 1
        row = self._add_summary_row(section, "Ap initial", f"{Ap_initial*1e6:.1f} mm²", row)
        row = self._add_summary_row(section, "Ap final", f"{Ap_final*1e6:.1f} mm²", row)
        row = self._add_summary_row(section, "Dp initial", f"{Dp_initial*1000:.2f} mm", row)
        row = self._add_summary_row(section, "Dp final", f"{Dp_final*1000:.2f} mm", row)
    ...
    def _add_section_header(self, parent, text, row):
        ...
        ).grid(row=row, column=0, columnspan=2, sticky='w', padx=10, pady=(8, 2))
    
    def _add_summary_row(self, parent, label, value, row):
        """Add a label-value pair to the summary and return next row"""
        tk.Label(
            parent, text=label + ":", font=('Arial', 9),
            bg=COLORS['bg_medium'], fg=COLORS['text_color'], anchor='e'
        ).grid(row=row, column=0, sticky='e', padx=(20, 5), pady=1)
        tk.Label(
            parent, text=value, font=('Arial', 9, 'bold'),
            bg=COLORS['bg_medium'], fg=COLORS['accent'], anchor='w'
        ).grid(row=row, column=1, sticky='w', padx=(5, 10), pady=1)
        return row + 1
    ...
    def _create_geometry_tab(self, parent):
        ...
        # Color map for time evolution (Blue → Red without plt.cm)
        n_snapshots = len(self.geometry_history)
        
        # Plot each snapshot with Blue→Red gradient
        for i, (x, y, t) in enumerate(self.geometry_history):
            if x is not None and y is not None and len(x) > 0:
                ...
                # Calculate color: Blue (0,0,1) → Red (1,0,0)
                ratio = i / max(n_snapshots - 1, 1)
                color = (ratio, 0, 1 - ratio)  # RGB tuple
        ...
        # Plot initial (blue) and final (red) with emphasis
        if n_snapshots > 0:
            x0, y0, t0 = self.geometry_history[0]
            if x0 is not None and len(x0) > 0:
                x0_closed = np.append(x0, x0[0])
                y0_closed = np.append(y0, y0[0])
                ax.plot(x0_closed * 1000, y0_closed * 1000, 
                       'b-', linewidth=2.5, label=f't = {t0:.2f} s (initial)')
        
        if n_snapshots > 1:
            xf, yf, tf = self.geometry_history[-1]
            if xf is not None and len(xf) > 0:
                xf_closed = np.append(xf, xf[0])
                yf_closed = np.append(yf, yf[0])
                ax.plot(xf_closed * 1000, yf_closed * 1000, 
                       'r-', linewidth=2.5, label=f't = {tf:.2f} s (final)')
        
        # Chamber wall - ALWAYS use D_chamber from results if available
        theta = np.linspace(0, 2*np.pi, 100)
        D_ch = None
        D_ch_source = "input"
        
        # Try to get D_chamber from results (now should always be there)
        if 'D_chamber' in self.results:
            D_chamber_list = self.results.get('D_chamber', [])
            if D_chamber_list and len(D_chamber_list) > 0 and D_chamber_list[0] is not None:
                D_ch = D_chamber_list[0]
                D_ch_source = "input"
        
        # Fallback: estimate from geometry (with warning)
        if D_ch is None or D_ch <= 0:
            max_extent = 0
            for x, y, _ in self.geometry_history:
                if x is not None and len(x) > 0:
                    max_extent = max(max_extent, np.max(np.abs(x)), np.max(np.abs(y)))
            D_ch = max_extent * 2.2  # Estimate with margin
            D_ch_source = "estimated"
            print(f"[WARNING] D_chamber not found in results, using estimated value: {D_ch*1000:.2f} mm")
        
        # Consistency check: compare D_chamber with max geometry extent
        max_geom_radius = 0
        for x, y, _ in self.geometry_history:
            if x is not None and len(x) > 0:
                max_geom_radius = max(max_geom_radius, np.max(np.hypot(x, y)))
        
        if max_geom_radius > D_ch / 2 * 0.98:  # Geometry exceeds 98% of chamber radius
            print(f"[WARNING] Grain geometry (r={max_geom_radius*1000:.2f}mm) exceeds chamber radius ({D_ch/2*1000:.2f}mm)!")
        
        # Plot chamber wall
        chamber_label = f'Chamber wall (D={D_ch*1000:.1f}mm, {D_ch_source})'
        ax.plot((D_ch/2)*1000*np.cos(theta), (D_ch/2)*1000*np.sin(theta), 
               'k--', linewidth=2, alpha=0.7, label=chamber_label)
        
        ax.set_xlabel('x [mm]', fontsize=11, color=COLORS['text_color'])
        ax.set_ylabel('y [mm]', fontsize=11, color=COLORS['text_color'])
        ax.set_title('Grain Geometry Evolution (Blue=Initial → Red=Final)', fontsize=12, 
                    color=COLORS['text_color'], fontweight='bold')
        ax.set_aspect('equal')
