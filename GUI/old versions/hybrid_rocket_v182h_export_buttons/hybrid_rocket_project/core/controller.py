...
class ApplicationController:
    ...
    def prepare_mission_inputs(self, mission_data: Dict) -> Dict:
        ...
        # Get expansion ratio from mission data (priority) or config
        if 'expansion_ratio' in mission_data and mission_data['expansion_ratio']:
            eps = mission_data['expansion_ratio']
        else:
            eps_str = config.inputs.get('Nozzle_ExpansionRatio', 'adapt')
            if eps_str and str(eps_str).lower() != 'adapt':
                try:
                    eps = float(eps_str)
                except:
                    eps = 'adapt'
    ...
    def _generate_grain_geometry(self, mission_data: Dict, Dport: float):
        ...
        # Regular Polygon: simple n-sided polygon with vertices at outer_r
        if preset == 'Regular Polygon':
            ...
            print(f"Regular Polygon grain:")
            print(f"  N_sides: {n_sides}")
            print(f"  Points: {len(x)}")
            print(f"  Deq_normalized: {Deq_norm*1000:.2f} mm")
            print(f"  Dport_target: {Dport*1000:.2f} mm")
            print(f"  Scale_factor: {scale_factor:.4f}")
