...
class OptimizationOutputPage:
    ...
    def create_page(self):
        """Create the optimization output page content with vertical scroll"""
        # Create outer frame for scroll functionality
        outer_frame = tk.Frame(self.parent, bg=COLORS['bg_dark'])
        outer_frame.pack(fill=tk.BOTH, expand=True)
        
        # Create canvas with vertical scrollbar
        self.page_canvas = tk.Canvas(outer_frame, bg=COLORS['bg_dark'], highlightthickness=0)
        v_scrollbar = tk.Scrollbar(outer_frame, orient=tk.VERTICAL, command=self.page_canvas.yview)
        
        # Scrollable frame inside canvas
        self.scrollable_frame = tk.Frame(self.page_canvas, bg=COLORS['bg_dark'])
        
        # Configure canvas
        self.page_canvas.configure(yscrollcommand=v_scrollbar.set)
        ...
        self.canvas_window = self.page_canvas.create_window((0, 0), window=self.scrollable_frame, anchor='nw')
        ...
        self.page_canvas.bind('<Configure>', update_canvas_width)
        
        # Enable mousewheel scrolling on the entire page
        def _on_mousewheel(event):
            self.page_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")
        
        def _bind_mousewheel(event):
            self.page_canvas.bind_all('<MouseWheel>', _on_mousewheel)
        
        def _unbind_mousewheel(event):
            self.page_canvas.unbind_all('<MouseWheel>')
        
        self.page_canvas.bind('<Enter>', _bind_mousewheel)
        self.page_canvas.bind('<Leave>', _unbind_mousewheel)
        
        # ===== PAGE CONTENT INSIDE SCROLLABLE FRAME =====
        ...
        main_container = tk.Frame(self.scrollable_frame, bg=COLORS['bg_dark'])
    ...
    def create_tab_content(self, tab_frame, tab_name):
        ...
        # Check if this is a single plot tab (like Gox Analysis) - make it larger
        is_single_plot = tab_def.get('single_plot', False)
        if is_single_plot:
            fig = Figure(figsize=(10, 7), dpi=100)  # Larger figure for single plot
        else:
    ...
    def create_buttons(self):
        ...
        # Button frame inside scrollable area
        button_frame = tk.Frame(self.scrollable_frame, bg=COLORS['bg_dark'])
        ...
        self.export_plots_button = tk.Button(
            ...
            bg='#4CAF50',
            fg='white',
            command=lambda: self.export_plots_format('png'),
            cursor='hand2',
            padx=12,
            pady=8,
            state=tk.DISABLED
        )
        self.export_plots_button.pack(side=tk.LEFT, padx=5)
        
        # Export Plots PDF button
        self.export_pdf_button = tk.Button(
            ...
            bg='#2196F3',
            fg='white',
            command=lambda: self.export_plots_format('pdf'),
            cursor='hand2',
            padx=12,
            pady=8,
            state=tk.DISABLED
        ...
        # Export to PLT button
        self.export_plt_button = tk.Button(
            button_frame,
            text="📈 .plt",
            font=FONTS['button'],
            bg='#FF9800',
        ...
        self.export_plt_button.pack(side=tk.LEFT, padx=5)
    ...
    def display_results(self, results, ranges=None, gox_min=100.0, gox_max=800.0):
        ...
        self.export_plt_button.config(state=tk.NORMAL)
    ...
    def export_plots(self):
        """Export current plots to PNG image files (legacy)"""
        self.export_plots_format('png')
    
    def export_plots_format(self, format='png'):
        ...
        if directory:
            try:
                ...
                for tab_name, fig_data in self.tab_figures.items():
                    fig = fig_data['figure']
                    filename = f"opt_{tab_name.replace(' ', '_').replace('&', 'and')}.{format}"
    ...
    def export_to_plt(self):
        """Export optimization results to .plt file (columnar data format)"""
        if not self.results:
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            title="Export Optimization Data (.plt)"
        )
        ...
        try:
            # Get ranges
            dport_range = self.ranges.get('Dport_Dt_range', np.array([]))
            dinj_range = self.ranges.get('Dinj_Dt_range', np.array([]))
            lc_range = self.ranges.get('Lc_Dt_range', np.array([]))
            
            # Parameters to export
            export_keys = ['pc_array', 'Is_array', 'Ivac_array', 'Gox_array', 'MR_array', 
                          'Tc_array', 'gamma_array', 'eps_array', 'CF_array', 'cs_array']
            
            with open(filepath, 'w') as f:
                # Header
                f.write("# Optimization Results - Plot Data\n")
                f.write(f"# Grid size: {len(dport_range)} x {len(dinj_range)} x {len(lc_range)}\n")
                f.write(f"# Dport/Dt range: {dport_range[0]:.3f} - {dport_range[-1]:.3f}\n")
                f.write(f"# Dinj/Dt range: {dinj_range[0]:.3f} - {dinj_range[-1]:.3f}\n")
                f.write(f"# Lc/Dt range: {lc_range[0]:.3f} - {lc_range[-1]:.3f}\n")
                f.write("#\n")
                f.write("# Columns: Dport_Dt  Dinj_Dt  Lc_Dt  " + "  ".join(export_keys) + "\n")
                f.write("#\n")
                
                # Data rows - iterate through all combinations
                for i, dport in enumerate(dport_range):
                    for j, dinj in enumerate(dinj_range):
                        for k, lc in enumerate(lc_range):
                            row = [f"{dport:.4f}", f"{dinj:.4f}", f"{lc:.4f}"]
            ...
            n_points = len(dport_range) * len(dinj_range) * len(lc_range)
            messagebox.showinfo("Success", 
                f"Optimization data exported to:\n{filepath}\n\n"
                f"Data points: {n_points}\n"
                f"Columns: 3 parameters + {len(export_keys)} results\n\n"
                "Load in matplotlib:\n"
                "  data = np.loadtxt('file.plt')\n"
                "  # data[:,0]=Dport/Dt, data[:,3]=pc, etc.")
