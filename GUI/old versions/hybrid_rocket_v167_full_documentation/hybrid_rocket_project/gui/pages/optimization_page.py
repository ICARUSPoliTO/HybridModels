...
DEFAULT_VALUES = {
    ...
    'Gox_min': '100',           # Minimum Gox [kg/(s·m²)] - extinction limit
    'Gox_max': '800',           # Maximum Gox [kg/(s·m²)] - blow-off limit
}
...
class OptimizationPage:
    ...
    def create_page(self):
        ...
        # Gox limits section
        self.create_gox_limits_section(main_frame)
    ...
    def create_gox_limits_section(self, parent):
        """Create Gox (oxidizer mass flux) limits section"""
        section = tk.LabelFrame(
            parent, 
            text="Oxidizer Mass Flux Limits (Gox)", 
            font=FONTS['section'],
            bg=COLORS['bg_medium'], 
            fg=COLORS['text_color']
        )
        ...
        # Info about Gox
        info_text = ("Gox = Oxidizer mass flux [kg/(s·m²)]\n"
                    "Solutions outside this range will be excluded from results.\n"
                    "Typical range: 100-800 kg/(s·m²)")
        ...
        # Gox min
        create_input_field(section, "Optimization_Gox_min", 
                          "Gox min [kg/(s·m²)]:", self.inputs, 
                          default=DEFAULT_VALUES['Gox_min'], row=1)
        
        # Gox max
        create_input_field(section, "Optimization_Gox_max", 
                          "Gox max [kg/(s·m²)]:", self.inputs, 
                          default=DEFAULT_VALUES['Gox_max'], row=2)
        
        # Reference info
        ref_frame = tk.Frame(section, bg=COLORS['bg_medium'])
        ref_frame.grid(row=3, column=0, columnspan=2, sticky='w', padx=10, pady=10)
        
        ref_text = ("Reference limits:\n"
                   "  • Gox < 100: Risk of extinction (flame too weak)\n"
                   "  • Gox > 800: Risk of blow-off (flame detachment)\n"
                   "  • Optimal: 200-500 kg/(s·m²) for most propellants")
