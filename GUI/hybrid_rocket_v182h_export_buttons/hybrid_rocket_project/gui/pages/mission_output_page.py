...
class MissionOutputPage:
    ...
    def create_export_section(self, parent):
        ...
        # Data export buttons
        tk.Label(section, text="Data:", font=('Arial', 9, 'bold'),
        ...
        # Separator
        tk.Label(section, text="  |  ", font=('Arial', 10),
                bg=COLORS['bg_dark'], fg=COLORS['text_muted']
        ).pack(side=tk.LEFT)
        
        # Plot export buttons
        tk.Label(section, text="Plots:", font=('Arial', 9, 'bold'),
                bg=COLORS['bg_dark'], fg=COLORS['text_color']
        ).pack(side=tk.LEFT, padx=(5, 2))
        ...
        tk.Button(
            ...
            bg='#4CAF50',
            fg='white',
            command=lambda: self.export_plots('png'),
            cursor='hand2',
            padx=10, pady=6
        ).pack(side=tk.LEFT, padx=3)
        ...
        tk.Button(
            ...
            bg='#2196F3',
            fg='white',
            command=lambda: self.export_plots('pdf'),
            cursor='hand2',
            padx=10, pady=6
        ).pack(side=tk.LEFT, padx=3)
        ...
        tk.Button(
            ...
            bg='#9C27B0',
            fg='white',
            command=lambda: self.export_plots('svg'),
            cursor='hand2',
            padx=10, pady=6
        ).pack(side=tk.LEFT, padx=3)
        ...
        # PLT file export (matplotlib pickle)
        tk.Button(
            section,
            text="📈 Save .plt",
            font=FONTS['button'],
    ...
    def create_plots_section(self, parent):
        ...
        if self.geometry_history and len(self.geometry_history) > 0:
            ...
            for x, y, t in self.geometry_history:
                if x is not None and y is not None and len(x) >= 3:
                    valid_geometry = True
                    break
            ...
            if valid_geometry:
                ...
                # Show info message that geometry could not be visualized
                messagebox.showinfo(
                    "Geometry Visualization",
                    "Grain geometry evolution could not be visualized.\n\n"
                    "The geometry data is incomplete or invalid.\n"
                    "Please check the grain geometry configuration."
                )
        else:
            # No geometry history at all - check if it should have been available
            if 'x' in self.results and 'y' in self.results:
                x_list = self.results.get('x', [])
                y_list = self.results.get('y', [])
                if not x_list or not y_list or all(x is None for x in x_list):
                    messagebox.showinfo(
                        "Geometry Visualization",
                        "Grain geometry evolution is not available.\n\n"
                        "The simulation did not produce valid geometry data.\n"
                        "This may occur if:\n"
                        "• The grain configuration is incorrect\n"
                        "• The simulation terminated before burning started\n"
                        "• The geometry preset is not supported"
                    )
    ...
    def _create_geometry_tab(self, parent):
        ...
        # Color map for time evolution (Blue → Red gradient without plt.cm)
    ...
    def export_plots(self, format='png'):
        ...
        # Ask for directory
        directory = filedialog.askdirectory(title=f"Select folder to save {format.upper()} plots")
        if not directory:
            return
        ...
        try:
            ...
            for tab_name, params in self.PLOT_TABS.items():
                ...
                n_plots = len(valid_params)
                fig = Figure(figsize=(12, 4 * n_plots), dpi=150)
                time = np.array(self.time_data)
                ...
                for i, param in enumerate(valid_params):
                    ...
                    # Get data
                    data = self.results.get(param, [])
                    y_data = np.array([v if v is not None and np.isfinite(v) else np.nan for v in data])
                    ...
                    ax.plot(time[:len(y_data)], y_data, 'b-', linewidth=1.5)
                    ax.set_xlabel('Time [s]', fontsize=10)
                    ax.set_ylabel(f'{label} [{unit}]', fontsize=10)
                    ax.set_title(f'{label} vs Time', fontsize=11, fontweight='bold')
                    ax.grid(True, alpha=0.3)
                
                fig.tight_layout()
                
                # Save file
                filename = f"mission_{tab_name.replace(' ', '_').replace('&', 'and')}.{format}"
                filepath = os.path.join(directory, filename)
                fig.savefig(filepath, format=format, dpi=150, bbox_inches='tight',
                           facecolor='white', edgecolor='none')
                saved_files.append(filename)
            
            # Save geometry plot if available
            if self.geometry_history and len(self.geometry_history) > 0:
                fig_geom = Figure(figsize=(10, 10), dpi=150)
                ax = fig_geom.add_subplot(111)
                ...
                for i, (x, y, t) in enumerate(self.geometry_history):
                    if x is not None and y is not None and len(x) > 0:
                        ...
                        alpha = 0.3 + 0.7 * ratio
                        ax.plot(x_closed * 1000, y_closed * 1000, color=color, linewidth=1.5, alpha=alpha)
                
                ax.set_xlabel('x [mm]', fontsize=11)
                ax.set_ylabel('y [mm]', fontsize=11)
                ax.set_title('Grain Geometry Evolution (Blue=Initial → Red=Final)', fontsize=12, fontweight='bold')
                ax.set_aspect('equal')
                ax.grid(True, alpha=0.3)
                
                fig_geom.tight_layout()
                filename = f"mission_grain_evolution.{format}"
                filepath = os.path.join(directory, filename)
                fig_geom.savefig(filepath, format=format, dpi=150, bbox_inches='tight',
                                facecolor='white', edgecolor='none')
                saved_files.append(filename)
            
            messagebox.showinfo("Success", 
                f"Exported {len(saved_files)} plots to:\n{directory}\n\n"
                f"Files: {', '.join(saved_files)}")
                
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export plots:\n{str(e)}")
    
    def export_to_plt(self):
        """Export plot data to .plt file (compatible with gnuplot/matplotlib)
        
        Creates a text file with columnar data that can be loaded by
        matplotlib, gnuplot, or other plotting software.
        """
        if not self.results or not self.time_data:
            messagebox.showerror("Error", "No results to export.")
            return
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            filetypes=[("Plot data files", "*.plt"), ("Data files", "*.dat"), ("All files", "*.*")],
            title="Export Plot Data (.plt)"
        )
        ...
        try:
            # Get all numeric parameters (exclude geometry arrays)
            export_params = []
            for key in self.results.keys():
                if key not in ['x', 'y', 'entropies', 'masses', 'pressures', 'temperatures']:
                    values = self.results[key]
                    if values and len(values) > 0:
                        # Check if it's numeric (not a list of arrays)
                        first_val = values[0]
                        if first_val is None or isinstance(first_val, (int, float)):
                            export_params.append(key)
            
            with open(filepath, 'w') as f:
                # Header with metadata
                f.write("# Mission Simulation Plot Data\n")
                f.write(f"# Generated by Hybrid Rocket Simulator\n")
                f.write(f"# Time steps: {len(self.time_data)}\n")
                f.write("#\n")
                
                # Column headers
                headers = ['time'] + export_params
                f.write("# Columns:\n")
                for i, h in enumerate(headers):
                    info = self.PARAM_INFO.get(h, (h, '-'))
                    unit = info[1] if len(info) > 1 else '-'
                    f.write(f"#   {i+1}: {h} [{unit}]\n")
                f.write("#\n")
                f.write("# " + "\t".join(headers) + "\n")
                
                # Data rows
                for i in range(len(self.time_data)):
                    row = [f"{self.time_data[i]:.6e}"]
                    for param in export_params:
                        try:
                            val = self.results[param][i] if i < len(self.results[param]) else 0
            ...
            messagebox.showinfo("Success", 
                ...
                f"Rows: {len(self.time_data)}\n\n"
                "Load in matplotlib:\n"
                "  data = np.loadtxt('file.plt')\n"
