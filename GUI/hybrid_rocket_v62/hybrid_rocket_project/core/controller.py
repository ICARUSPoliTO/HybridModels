...
class ApplicationController:
    ...
    def _generate_grain_geometry(self, mission_data: Dict, Dport: float):
        ...
        if preset == 'Custom (from CSV)':
            ...
            if custom_x is not None and custom_y is not None and len(custom_x) > 2:
                x_norm = np.array(custom_x)
                y_norm = np.array(custom_y)
                ...
                print(f"Custom CSV geometry loaded:")
                ...
                print("Warning: Custom geometry not loaded, falling back to cylindrical")
                preset = 'Cylindrical'
