...
class MissionPage:
    ...
    def _on_grain_preset_change(self, event=None):
        ...
        # === Regular Polygon OR Custom Polygon: show only n_sides and outer_radius ===
        ...
        # === All other presets: show all fields ===
        else:
            show_frame(self.n_sides_frame, row=1, col=0, colspan=2)
    ...
    def _on_auto_toggle(self):
        ...
        if self.auto_dt_var and self.auto_dt_var.get():
            # Auto enabled - calculate and lock field
            self._auto_calculate_dt()
    ...
    def _auto_calculate_dt(self):
        ...
        try:
            ...
            # Get grain outer radius to estimate Dport
            outer_r = float(self.inputs.get('Mission_grain_outer_radius', 
                           tk.Entry()).get() or '0.04')
            
            # Estimate Dport ~ 2 * outer_r (simplified)
            dport = 2 * outer_r
            
            # Calculate Dt
            dt = dport / dport_dt if dport_dt > 0 else 0.03
