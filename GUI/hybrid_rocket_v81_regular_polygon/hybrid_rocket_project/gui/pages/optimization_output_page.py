        ...
        for i, var_name in enumerate(variables):
            ...
            try:
                ...
                if var_name == 'Gox_array':
                    ...
                    # Add labels for limit lines
                    if len(cs_min.allsegs[0]) > 0:
                        ax.clabel(cs_min, inline=True, fontsize=9, fmt=f'Gox_min={self.gox_min:.0f}',
                                 colors='red')
                    if len(cs_max.allsegs[0]) > 0:
                        ax.clabel(cs_max, inline=True, fontsize=9, fmt=f'Gox_max={self.gox_max:.0f}',
                                 colors='red')
                    ...
                    if hasattr(self, 'optimal_indices') and self.optimal_indices is not None:
                        ...
                        if opt_k == slice_idx:
                            ...
                            ax.plot(opt_dport, opt_dinj, 'g*', markersize=15, 
                                   markeredgecolor='white', markeredgewidth=1.5,
                                   label='Optimal (max Ivac)')
